...
import { FileTreeItem, FileTreeDirectory, FileTreeFile } from "../../types/file-tree"


export default function Zip() {

    const inputTreeRef = useRef<FileTreeDirectory>({ name: "root", type: "dir", children: [] })
    const [inputTreeRefresh, setInputTreeRefresh] = useState<number>(0)
    const workerRef = useRef<Worker | null>(null)

    /* File Tree Operations */
    function fileTreeInsertItem(path: string, item: FileTreeItem) {
        ...
        while (pathList.length > 1) {
            ...
            const folderCandidates = currentNode.children
                .filter(item => {
            ...
            if (folderCandidates.length > 0) {
                ...
                currentNode = currentNode.children[currentNode.children.length - 1] as FileTreeDirectory
            }
    ...
    function fileTreeRemoveItem(itemName: string) {

        let subItems = inputTreeRef.current.children
        subItems = subItems.filter(
            (value) => {
                return value.name !== itemName
            }
        );
        inputTreeRef.current.children = subItems;
        setInputTreeRefresh(Math.random())
    }

    /* File Drop Functionality */
    async function fileEntryToFiles(entries: FileSystemEntry[], currentPath: string = "") {
                    ...
                    new Promise(
                        (resolve, reject) =>{
                            directoryEntry.createReader().readEntries(
                                async data => {
                                    const subEntries = await fileEntryToFiles(data, `${currentPath}/${directoryEntry.name}`)
    ...
    async function handleFileDrop(event: React.DragEvent<HTMLDivElement>) {
        ...
        if (fileEntries) {
            const files = await fileEntryToFiles(fileEntries as FileSystemEntry[])
            for (const [path, file] of files) {
                fileTreeInsertItem(path.substring(1), 
                    {
                        type: "file", 
                        name: file.name, 
                        data: file
        ...
        input.onchange = function (event) {
            ...
            if (element.files) {
                ...
                for (const file of inputFiles) {
                    ...
                    fileTreeInsertItem(file.webkitRelativePath, fileItem)
                }
            }
            input.remove()
        }
    ...
    async function compressFiles() {
        ...
        function buildFileMap(root: FileTreeDirectory, map: Map<string, Blob> = new Map(), 
            path: string = "") {

            for (const child of root.children) {
                if (child.type === "dir") {
                    const subMap = buildFileMap(child as FileTreeDirectory, map, path.concat(`${child.name}/`))
                    subMap.forEach(
                        (value, key) => {
                            map.set(key, value)
                        }
    ...
    return (
        <>
            <div className="flex flex-col gap-4">
                ...
                items-center justify-center h-64 text-3xl flex-col gap-3" onDrop={ async (e) => { await handleFileDrop(e) }} onDragOver={handleDragOver}>
                    Drag and Drop OR
                    <motion.button
                    ...
                    overflow-y-auto max-h-24 md:max-h-64">
                        {
                            inputTreeRef.current.children.map((item, index) => {
                                return (
                                    <FileCard key={index} file={item.name} onDelete={
                                        () => {
                                            
                                            fileTreeRemoveItem(item.name)
                                        }
                                    }/>
                                )
                            })
